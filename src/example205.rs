fn shuffle_array(nums: &[u32]) -> Result<Vec<u32>, &str> {
    ...
    let mut first_half = nums.iter().take(n);
    let mut second_half = nums.iter().skip(n);
    let mut shuffled = Vec::with_capacity(double_n);

    for _ in 0..n {
        shuffled.push(first_half.next().expect("Valid item").to_owned());
        shuffled.push(second_half.next().expect("Valid item").to_owned());
    }
