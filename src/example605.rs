...
fn min_moves_to_seat(seats: &[u32], students: &[u32]) -> Result<u32, &'static str> {
    ...
    let mut sum_of_dist = 0_u32;

    remaining_students.iter().for_each(|student| {
        let (nearest_seat_idx, nearest_seat) = remaining_seats
            .iter()
            .enumerate()
            .min_by_key(|(_, seat)| student.abs_diff(**seat))
            .expect(
                "Checked length of *seats* and *students* are equal, so at least one seat exist",
            );

        sum_of_dist += student.abs_diff(*nearest_seat);
        remaining_seats.remove(nearest_seat_idx);
    });
